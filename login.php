...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($password)) {
        ...
        $doctor = new Doctor();
        $result = $doctor->authenticate($email, $password);
        ...
        if ($result) {
            ...
            $doctor->updateDoctorStatus($result['id'], 'online');
...
function updateDoctorStatus($id, $status) {
    if (($handle = fopen("doctors.csv", "r")) !== FALSE) {
        // Read header
        ...
        // Write back to file
        if (($handle = fopen("doctors.csv", "w")) !== FALSE) {
...
<head>
    ...
    <title>Doctor Login - Availability System</title>
    ...
    <div class="login-container">
        <form class="login-form" method="POST" action="">
            ...
            <div style="text-align: right; margin-bottom: 1rem;">
                <a href="doctor_forgot_password.php" style="color: var(--secondary-color); text-decoration: none; font-size: 0.9rem;">
